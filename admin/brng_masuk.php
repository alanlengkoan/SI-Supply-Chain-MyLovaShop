    ...
    if ($datkod) {
     ...
     <body class="nav-md" onload="setInterval('reloadwaktu()');">
      ...
      if (isset($_POST['masuk'])) {
        ...
        $sql      = "SELECT * FROM dta_trnsaksi_brng_masuk WHERE id_transaksi = '$idtrns'";
        ...
        if ($row = $q_idtrns->fetch_row()) {
          ...
          $query  = "INSERT INTO dta_trnsaksi_brng_masuk (id_transaksi, id_supplier, nmasup, kd_barang, nmabar,
                     jumlah, harga, total, waktu) VALUES ('$idtrns', '$idsup', '$nmasup', '$kdbar',
                     '$nmabar', '$jmlah', '$hrga', '$total', '$waktu')";
          $result = $mysqli->query($query);

          echo "<script>
                  window.location=(href='dt_barangmasuk.php?&simpan')
                </script>";
        }
      }
